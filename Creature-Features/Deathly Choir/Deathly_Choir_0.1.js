...
async function doOnUse() {
    ...
    for (let i = 0; i < inRangeCount; i++) {
        ...
        if (save.total < SAVE_DC) {
            ...
            damaged.push(tToken)
            failNames += `+ <b>${tToken.name}</b> with a ${save.total} (${save._formula})<br>`
        } else {
            madeNames += `- <b>${tToken.name}</b> with a ${save.total} (${save._formula})<br>`
            damaged.push(tToken)
    ...
    MidiQOL.applyTokenDamage(
        [{damage: damageRoll.total, type: DAMAGE_TYPE}], 
        damageRoll.total,
         new Set(failSaves), 
         aItem, 
         new Set());
    ...
    MidiQOL.applyTokenDamage([{damage: halfdam, type: DAMAGE_TYPE}], halfdam, new Set(madeSaves),
        aItem.name, new Set());
    //----------------------------------------------------------------------------------
