const MACRONAME = "Quasit_Shape_Change.0.1.js"
/*********1*********2*********3*********4*********5*********6*********7*********8*********9*********0
 * Implment Quasit Shape Change Ability.  Changing displayed token image and movement per RAW, except
 * token does not revert on death and nothing is done with equipment.
 * 
 * This is based on Imp shape change
 * 
 *   Quasit can use its action to polymorph into a beast form that resembles:
 *    1. bat (speed 10 ft. fly 40 ft.),
 *    2. centipede (40 ft., climb 40 ft.), 
 *    3. toad (40 ft., swim 40 ft.), 
 ...
 * 
 * PRECONDITIONS
 * - The directory containing the token image for the Quasit must also contain images for other 
 *   shapes and they must all have the same file extension.
 * 
 * 09/23/22 0.1 Creation of Macro
 *********1*********2*********3*********4*********5*********6*********7*********8*********9*********/ 
...
//
let SHAPES = ["Quasit", "Bat", "Centipede", "Toad"]
//---------------------------------------------------------------------------------------------------
...
async function pickShapeCallBack(selection) {
    ...
    // Typical image data: aToken.data.img: 'Tokens/Monsters/Quasit/Quasit.png'
    //
    ...
    //-------------------------------------------------------------------------------------------------
    // Does the NEW_IMG file exist in the game data directory?  If not, share bad news and just retain 
    // the original image
    //
    ...
    switch (selection) {
        ...
        case "Quasit": moveObj.walk = 40; break;
        case "Bat": moveObj.fly = 40; moveObj.walk = 10; break;
        case "Centipede": moveObj.climb = 40; moveObj.walk = 40; break;
        case "Toad": moveObj.swim = 40; moveObj.walk = 40; break;
