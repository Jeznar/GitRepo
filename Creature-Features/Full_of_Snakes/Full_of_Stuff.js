const MACRONAME = "Full_of_Stuff.0.1.js"
const TL = 0;                               // Trace Level for this macro
/*********1*********2*********3*********4*********5*********6*********7*********8*********9*********0*********1*********2*********3*
 * Implement the full of snakes/worms abilities
 * 
 * This macro is intended to be called from the macro drawer in place of an ItemMacro.  It uses the item name to adjust the types of 
 * creatures that will be summoned.  As of its creation it is used by: 
 * - Full of Snakes 
 * - Full of Worms
 *  
 *   When the corpse is reduced to 0 hit points, it splits open, disgorging a  Swarm of Poisonous Snakes that attack immediately.
 * 
 * 10/05/23 0.1 Created from Full_of_Snakes.0.2.js
 *********1*********2*********3*********4*********5*********6*********7*********8*********9*********0*********1*********2*********3*/
...
//
function minionList() {
    let minions = ['Swarm of Poisonous Snakes', 'Swarm of Garden Snakes', 'Swarm of Constricting Snakes' ]
    if (aItem.name.toLowerCase().endsWith('worms')) minions = ['Swarm of Centipedes', 'Swarm of Insects', 'Swarm of Maggots', 
        'Swarm of Rot Grubs', 'Swarm of Scarabs', 'Swarm of Spiders', 'Swarm of Wasps' ]
    return minions
}
const MINIONS = minionList()
//-----------------------------------------------------------------------------------------------------------------------------------
...
async function doOnUse(options = {}) {
    ...
    //-------------------------------------------------------------------------------------------------------------------------------
    // 
    postResults(`${aToken.name} bursts open and an angry ${MINION_DATA.data.token.name} pours forth.`)
