const macroName = "Toggle_Three-Quarters_cover_0.2"
//###############################################################################
// READ THIS -  Requires Midi-QoL on use -or- Hotbar usage
/********************************************************************************
* Macro to toggle 3/4 Cover condition. 
...
* 12/09/21 0.2 JGB Add Code to remove 1/2 cover, if present
********************************************************************************/
const debug = true;
const effect = "Three-Quarters cover";
const otherEffect = "Half cover";
const gameRound = game.combat ? game.combat.round : 0;
const iconImage = args[0]?.item.img  || "icons/svg/target.svg"
let target = "";
...
async function processTarget(target,parm) {
    ...
    if (target.effects.find(ef => ef.data.label === effect)) {
        if (debug) console.log(` Three-Quarters_Cover turning off for ${target.data.token.name}`);
        ...
        if (!quiet) {
            the_message = `<em>${target.data.token.name} is <strong>no longer</strong> benefiting from Three-Quarters Cover!.</em>`;
            chatMessage = game.messages.get(args[0].itemCardId);
        }
    } else {
        if (debug) console.log(` Three-Quarters_Cover turning on for ${target.data.token.name}`);
        let effectData = [{
            label: effect,
            icon: iconImage,
        ...
        if (!quiet) {
            the_message = `<em>${target.data.token.name} is <strong>now benefiting</strong> from Three-Quarters_Cover!</em>`;
            chatMessage = game.messages.get(args[0].itemCardId);
        }
    }
    ...
    if (target.effects.find(ef => ef.data.label === otherEffect)) {
        ...
        /* game.cub.removeConditions(effect,target.data.token.name,{warn:true}); */
        the_message += `<br><em>${target.data.token.name} is <strong>no longer benefiting</strong> from Half Cover!</em>`;
