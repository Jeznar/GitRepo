const MACRONAME = "Move_Within_Vampyr_Mists.0.1.js"
/*********1*********2*********3*********4*********5*********6*********7*********8*********9*********0*********1*********2*********3*
 * On a failed save, move in random direction
 * 
...
async function doOnUse(options = {}) {
    ...
    //-------------------------------------------------------------------------------------------------------------------------------
    // 
    if (args[0].saveUuids.length === 1) {
        postResults(`${aToken.name} may move normally this turn.`)
        return
    }
    //-------------------------------------------------------------------------------------------------------------------------------
    // 
    ...
    msg = `<b>${aToken.name}</b> must use half of its movement to move <b>${DIRECTION[dRoll.total-1]}</b>.`
    postResults(msg)
    ...
    async function runVFX(target) {
        ...
        new Sequence()
            .effect()
            .file(`Icons_JGB/Monster_Features/move.png`)
            .attachTo(target)
            .scaleToObject(1.5)
            .opacity(1)
            .duration(6000)
