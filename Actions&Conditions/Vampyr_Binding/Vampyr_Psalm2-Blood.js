const MACRONAME = "Vampyr_Psalm2-Blood.0.1.js"
/*********1*********2*********3*********4*********5*********6*********7*********8*********9*********0*********1*********2*********3*
 * 2nd Psalm:  All Living must succeed on a DC 15 Charisma saving throw or lose 1 hit point at the end of each of your 
 *             turns until the ritual is complete.
 * 
 ...
 *    b. Apply an effect that implements a turn end DoT
 * 
...
async function psalm2(options = {}) {
    ...
    for (let i = 0; i < LIVINGS.length; i++) {
        ...
        // a. Roll saving throw continue if save made
        //
        // let friendlyCnt = LIVINGS.length
        msg = `Total of ${LIVINGS.length} ritual participants.<br>`
        // for (let j = 0; j < friendlyCnt; j++) {
        ...
        //-------------------------------------------------------------------------------------------------------------------------------
        // b. Apply a DoT effect
        //
        if (TL > 1) jez.trace(`${TAG} ${fToken.name} gets a DoT effect`)
        runVFX(fToken)
        if (TL > 1) jez.trace(`${TAG} Apply debuff that reduces speed by 5 to ${fToken.name}`)
        await jez.wait(100)
        let existingPsalm2 = await fToken.actor.effects.find(ef => ef.data.label === aItem.name);
        if (TL > 1) jez.trace(`${TAG} Existing effect (if any)`, existingPsalm2)
        if (existingPsalm2) await existingPsalm2.delete()
        await jez.wait(100)
        const CE_DESC = `Afflicted by Psalm of Terror: 2 Blood`
        let overTimeVal = `turn=end,label=Bleeding from the eyes,damageRoll=1,damageType=psychic`
        let effectData = {
            label: aItem.name,
            icon: 'Icons_JGB/Misc/blood2.svg',
            origin: L_ARG.uuid,
            disabled: false,
            flags: {
                convenientDescription: CE_DESC,
                dae: { specialDuration: ["longRest", "shortrest"] },
    ...
    else if (failSaves.length === 1) msg = `Until the ritual completes ${failSaves[0].name} loses 1 hit point at the end of each of 
        his/her turns until the ritual is complete.<br>`
    else msg = `Until the ritual completes all affected lose 1 hit point at the end of each of their turns until the ritual is complete.
        <br>`
    jez.addMessage(chatMessage, { color: "darkblue", fSize: 14, msg: msg, tag: "saves" })
    return
    //-----------------------------------------------------------------------------------------------------------------------------------
    ...
    async function runVFX(target) {
        ...
        new Sequence()
            .effect()
            .file(`Icons_JGB/Misc/blood2.svg`)
            .attachTo(target)
            .scaleToObject(1)
            .opacity(1)
            .duration(4000)
