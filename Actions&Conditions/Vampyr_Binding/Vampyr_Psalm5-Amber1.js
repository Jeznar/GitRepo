const MACRONAME = "Vampyr_Psalm5-Amber1.0.1.js"
/*********1*********2*********3*********4*********5*********6*********7*********8*********9*********0*********1*********2*********3*
 * 5th Psalm:  Whenever a character reads a stanza or casts a spell with a vocal component, the bloody mist attempts to choke them. 
 *             They must succeed on a DC 15 Charisma saving throw or they fail to read the stanza or cast the spell, and are 
 *             silenced until the start of their next turn.
 * 
...
/*********1*********2*********3*********4*********5*********6*********7*********8*********9*********0*********1*********2*********3*
 * 5th Psalm:  Whenever a character voices a stanza or casts a spell with a vocal component, the bloody mist attempts to choke them. 
 *             They must succeed on a DC 15 Charisma saving throw or they fail to read the stanza or cast the spell, and are 
 *             silenced until the start of their next turn.
 * 
 * This one needs to do the following:
...
async function psalm5(options = {}) {
    ...
    // Set Macro variables
    //
    const TEMP_ABILITY = 'Speak within Vampyr Mists'
    //--------------------------------------------------------------------------------------------------------------------------------
    // 1. Find all of the friendly tokens
    // 
    ...
    // 3. Post Results
    // 
    msg = `During the remainder of the ritual, any attempt at speech must be preceeded by using the new, feature, active action
    <b>${TEMP_ABILITY}</b>, failure on that ability silences the words that were to be spoken.`
    postResults(msg)
    return
}
/*********1*********2*********3*********4*********5*********6*********7*********8*********9*********0*********1*********2*********3*
