...
class jez {
  ...
  wait(ms) -- async call to waith for ms milliseconds`
     ...
     ***************************************************************************************************/
    static trace(...parms) {
        return jez.writeTrcLog("JezT", ...parms)
    }
    /***************************************************************************************************
     * Log
     * 
     ...
     ***************************************************************************************************/
    static log(...parms) {
        return jez.writeTrcLog("JezL", ...parms)
    }
    /***************************************************************************************************
...
`   // Back tick on its on line to make the console output better formatted
    ...
    * getRaceSubType
    *
    * Return the race subtype of the passed Actor5e, Token5e, or TokenDocument5e.  The value will be a 
    * lowercase string, which may be empty.  It is taken from a user input field, so garbage may be 
    * present.  For character tokens, return the race.
    * 
    * If passed a parm not of a supported type, return FALSE
    ***************************************************************************************************/
    static getRaceSubType(entity) {
        // function getRaceSubType(entity) {
        let objType = entity.constructor.name
        let subject = null
        if (objType === "Actor5e")          // perhaps entity is actor5e?  
            subject = entity                // point subject at the actor5e
