const MACRONAME = "Refresh_Item_On_Actors.1.2.js"
/*********1*********2*********3*********4*********5*********6*********7*********8*********9*********0
 ...
 * 05/14/23 1.2 Update to retain folder for item in side bar
 *********1*********2*********3*********4*********5*********6*********7*********8*********9*********/
...
async function main() {
    ...
    Dialog.confirm({
        ...
        <p>Certain fields can be retained from each actor's version of the item:
        </p><p>
        <ul>
...
async function updateItemInSidebar(tokenD, nameOfItem, typeOfItem) {
    ...
    console.log(`Item ${itemOrigin.name} on actor:`, itemOrigin)
    ...
    //-------------------------------------------------------------------------------------------------------------------------------
    // Snag the folder ID for the sidebar item, something like nT5wUN8fxvHYCTBI and set it into new version
    ...
    let itemObject = itemOrigin.toObject()
    itemObject.folder = itemInSidebar.data.folder               // Set folder for new object to the folder for sidebar object
    await itemInSidebar.update(itemObject)                      // Update reference to match source
    //-------------------------------------------------------------------------------------------------------------------------------
