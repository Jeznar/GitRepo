 ...
 * Modification of the ASE module code to correct problems found
 * 
 * 08/01/23 0.1 Add trace statements to checkCrosshairs and updated range logic
 *              - getContainedCustom() updated to use feet not a hybrid of tiles and feet
 *********1*********2*********3*********4*********5*********6*********7*********8*********9*********0*********1*********2*********3*/
...
export function getContainedCustom(tokenD, crosshairs) {
    ...
    const TAG = `ASE_utilityFunctions-getContainedCustom |`;                        // -Jez 23.08.01
    if (TL === 1) jez.trace(`${TAG} Called on ${tokenD.name}`);                     // -Jez 23.08.01
    let tokenCenter = getCenter(tokenD.data, tokenD.data.width);
    let tokenCrosshairsDist = canvas.grid.measureDistance(tokenCenter, crosshairs); // -Jez 23.08.01 units will be map "feet"
    let crosshairsDistance = crosshairs.data?.distance ?? crosshairs.distance;      // -Jez 23.08.01 units will be diameter in squares
    // Original math following line -Jez 23.08.01
    // let distanceRequired = (crosshairsDistance - 2.5) + (2.5 * tokenD.data.width); // -Jez 23.08.01
    //---------------------------------------------------------------------------------------------------------------------------------
    // Following code block added Updated math -Jez 23.08.01        
    //
    const FEET_PER_SQUARE = canvas.scene.data.gridDistance || 5     
    // Half the crosshairsDistance is radius of target area in squares      
    const DIST_ALLOWED = crosshairsDistance/2 * FEET_PER_SQUARE // Distance from middle of target to edge in feet
    const TARGET_PAD = 2.5 * tokenD.data.width                  // Distance from middle of target to edge of target in feet                                                  
    const EFFECT_DISTANCE = DIST_ALLOWED + TARGET_PAD           // Distance middle of target can be and still be hit
    if (TL > 1) jez.trace(`${TAG} ${tokenD.name}: Dist ${tokenCrosshairsDist}, Req ${EFFECT_DISTANCE}`)
    //---------------------------------------------------------------------------------------------------------------------------------
    // if ((tokenCrosshairsDist) < distanceRequired) {
    if ((tokenCrosshairsDist) < EFFECT_DISTANCE) {
        return true;
    }
    else {
        return false;
...
export async function checkCrosshairs(crosshairs) {
    const TL = 0;                                                               // -Jez 23.08.01
    const TAG = `ASE_utilityFunctions-checkCrosshairs |`;                       // -Jez 23.08.01
    if (TL > 0) jez.trace(`${TAG} Called, crosshairs: `, crosshairs);           // -Jez 23.08.01
    let collected;
    while (crosshairs.inFlight) {
        ...
        if (TL > 2) jez.trace(`${TAG} collected: `,collected);                  // -Jez 23.08.01
        ...
        if (TL > 2) jez.trace(`${TAG} crosshairs.label:   `,crosshairs.label);  // -Jez 23.08.01
        for await (let tokenD of collected) {
            let token = canvas.tokens.get(tokenD.id);
            if (TL > 3) jez.trace(`${TAG} token:     `,token);                  // -Jez 23.08.01
