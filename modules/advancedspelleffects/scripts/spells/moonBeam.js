 ...
 * Modification of the ASE module code to correct wrong sized moonbeam.  It was using a 5 foot diameter, should have been radius.
 * 
 * 07/10/23 0.1 Activated trace statements and doubled size of rendered tile.
 *********1*********2*********3*********4*********5*********6*********7*********8*********9*********0*********1*********2*********3*/
...
export class moonBeam {
    ...
    static async _updateToken(tokenDocument, updateData) {
        const isGM = utilFunctions.isFirstGM();                                 
        const TAG = `moonBeam_updateToken |`                                    // -Jez 7.10.23
        const TL = 0;                                                           // -Jez 7.10.23
        if (TL > 1) {                                                           // -Jez 7.10.23
            jez.trace(`${TAG} Is first GM: `, isGM);                            // -Jez 7.10.23
            jez.trace(`${TAG} Update Token tokenDocument: `, tokenDocument);    // -Jez 7.10.23
            jez.trace(`${TAG} Update Token updateData: `, updateData);          // -Jez 7.10.23
        }                                                                       // -Jez 7.10.23
        if (!isGM) return;
        ...
        if (TL > 1) {                                                           // -Jez 7.10.23
            jez.trace(`${TAG} inTiles:            `, inTiles);                  // -Jez 7.10.23
            jez.trace(`${TAG} moonbeamTiles:      `, moonbeamTiles);            // -Jez 7.10.23
            jez.trace(`${TAG} token:              `, token);                    // -Jez 7.10.23
            jez.trace(`${TAG} New Token Position: `, newTokenPosition);         // -Jez 7.10.23
        }                                                                       // -Jez 7.10.23
        ...
        for (let i = 0; i < moonbeamTiles.length; i++) {
            ...
            if (TL > 1) jez.trace(`${TAG} Moonbeam tile center: `, moonbeamTileCenter);     // -Jez 7.10.23
            let targetToBeamDist = utilFunctions.getDistanceClassic(newTokenPosition, moonbeamTileCenter);
            if (TL > 1) jez.trace(`${TAG} target to beam dist: `, targetToBeamDist);        // -Jez 7.10.23
            if (TL > 1) jez.trace(`${TAG} Required Distance: `, (((tokenDocument.data.width * canvas.grid.size) / 2) + (moonbeamTile.data.width / 2)));
            if (targetToBeamDist < (((tokenDocument.data.width * canvas.grid.size) / 2) + (moonbeamTile.data.width / 2))) {
                ...
                if (inTiles.includes(moonbeamTile.id)) {
                    jez.badNews(`${token.name} has already entered this tile this turn - ${moonbeamTile.id}`, 'info')
                }
                else {
                    jez.badNews(`${token.name} is entering the space of a moonbeam tile - ${moonbeamTile.id}`, 'info')
    ...
    static async _updateCombat(combat) {
        const isGM = utilFunctions.isFirstGM();
        const TAG = `moonBeam_updateCombat |`                                   // -Jez 7.10.23
        const TL = 0;                                                           // -Jez 7.10.23
        if (TL > 1) jez.trace(`${TAG} Is first GM: `, isGM);                    // -Jez 7.10.23
        if (!isGM) return;
        const moonbeamTiles = await Tagger.getByTag(`*-moonbeam`);
        if (moonbeamTiles.length == 0) return;
        if (TL > 1) {                                                           // -Jez 7.10.23
            jez.trace(`${TAG} "Updating Combat for ASE Moonbeam...`);           // -Jez 7.10.23
            jez.trace(`${TAG} combat: `, combat);                               // -Jez 7.10.23
        }                                                                       // -Jez 7.10.23
        ...
        for (let i = 0; i < moonbeamTiles.length; i++) {
            let moonbeamTile = moonbeamTiles[i];
            if (TL > 1) jez.trace(`${TAG} Moonbeam tile found: `, moonbeamTile);
            let effectOptions = moonbeamTile.getFlag("advancedspelleffects", "effectOptions") ?? {};
            ...
            if (TL > 1) jez.trace(`${TAG} target to beam dist: `, targetToBeamDist);
            if (targetToBeamDist < (((combatantToken.data.width * canvas.grid.size) / 2) + (moonbeamTile.data.width / 2))) {
                //check if tile exists in inTiles which is an array of tiles - Jez Huh?
                jez.badNews(`${combatantToken.name} is starting its turn in the space of a moonbeam tile - ${moonbeamTile.id}`, 'info')
                //add the tile to the inTiles array
    ...
    static async handleConcentration(casterActor, casterToken, effectOptions) {
        const TAG = `moonBeam_handleConcentration |`                            // -Jez 7.10.23
        const TL = 0;                                                           // -Jez 7.10.23
        if (TL > 1) jez.trace(`${TAG} Handling concentration removal for ASE moonBeam... `); // -Jez 7.10.23
    ...
    static async callBeam(data) {
        ...
        if (TL > 1) jez.trace(`${TAG} aseEffectOptions, `,aseEffectOptions);    // -Jez 7.10.23
        ...
        if (TL > 1) jez.trace(`${TAG} moonbeamTile, `,moonbeamTile);    // -Jez 7.10.23
        ...
        if (beamLoopSound != "") {
            ...
            if (TL > 1) jez.trace(`${TAG} Sound Created...`,sourceSound);    // -Jez 7.10.23
        }
        //---------------------------------------------------------------------------------------------------------------------------
        async function placeBeam(templateData, tokenId, beamAnim, effectOptions) {
            const TAG = `moonBeam_placeBeam |`                              // -Jez 7.10.23
            const TL = 0;                                                   // -Jez 7.10.23    
            let tileWidth = (templateData.width * canvas.grid.size * 2);    // -Jez 7.10.23 Added the * 2 to make it radius not dia.
            let tileHeight = (templateData.width * canvas.grid.size * 2 );  // -Jez 7.10.23 Added the * 2 to make it radius not dia.
            let tileX = templateData.x - (tileWidth / 2);
            let tileY = templateData.y - (tileHeight / 2);
            if (TL > 1) jez.trace(`${TAG} Values`,
                'tileWidth: ', tileWidth,
            ...
            if (TL > 1) jez.trace(`${TAG} Placing as tile: `, data);    // -Jez 7.10.23
            
            let createdTiles = await aseSocket.executeAsGM("placeTiles", data);
            if (TL > 1) jez.trace(`${TAG} ASE MOONBEAM: Moonbeam Tile Created:  `, createdTiles);    // -Jez 7.10.23
    ...
    static async activateBeam(token, effectOptions) {
        const TAG = `moonBeam_activateBeam |`                            // -Jez 7.10.23
        const TL = 0;                                                    // -Jez 7.10.23
      ...
      <div><img src="${token?.data?.img}" height="30" style="border:0px"></div>
        ...
        function customHalfRollChatCard(roll) {
            if (TL > 1) jez.trace(`${TAG} roll: `, roll);    // -Jez 7.10.23
        ...
        if (TL > 1) jez.trace(`${TAG} ROLL INFO: `, rollInfo);    // -Jez 7.10.23
        ...
        if (game.modules.get("midi-qol")?.active) {
            ...
            if (TL > 1) jez.trace(`${TAG} Rolls`,
                `fullDamageRoll: `, fullDamageRoll,
                `halfdamageroll: `, halfdamageroll,
                `saveRoll:       `, saveRoll);
    ...
    static async moveBeam(data) {
        const TAG = `moonBeam_moveBeam |`                                    // -Jez 7.10.23
        const TL = 0;                                                           // -Jez 7.10.23
        const casterActor = data.actor;
        const casterToken = canvas.tokens.get(data.tokenId);
        const spellItem = data.item;
        ...
        let moonbeamTiles = await Tagger.getByTag(`${casterToken.id}-moonbeam`);
        if (moonbeamTiles?.length == 0) return jez.badNews("Moonbeam not found.", 'error')
        let moonbeamTile = moonbeamTiles[0];

        let moonbeamLoc = await moonBeam.chooseBeamLocation(beamLoop);
        if (TL > 1) jez.trace(`${TAG} moonbeamLoc:        `, moonbeamLoc);    // -Jez 7.10.23
        if (TL > 1) jez.trace(`${TAG} moonbeamTile:       `, moonbeamTile);    // -Jez 7.10.23
        if (TL > 1) jez.trace(`${TAG} ASE EFFECT OPTIONS: `, aseEffectOptions);    // -Jez 7.10.23
