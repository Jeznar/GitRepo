 ...
 * Modification of the ASE module code to correct wrong size of placed tile.  It was correct while being placed but dropped 
 * to a 10 foot diameter tile (likely the default for that tile) after placement, seemingly by change in WarpGate
 * warpgate.crosshairs.show().
 * 
 * 08/02/23 0.1 Flip templateData.width to templateData.size
 *********1*********2*********3*********4*********5*********6*********7*********8*********9*********0*********1*********2*********3*/
...
export class darkness {
    ...
    static async createDarkness(midiData) {
        ...
        async function placeCloudAsTile(templateData, casterId, soundOptions) {
            ...
            let wallPoints = [];
            let walls = [];
            // tileWidth = (templateData.width * canvas.grid.size);     // -Jez 23.08.02 Original line
            tileWidth = (templateData.size * canvas.grid.size);         // -Jez 23.08.02 Fix by replacing width with size 
            // tileHeight = (templateData.width * canvas.grid.size);    // -Jez 23.08.02 Original line
            tileHeight = (templateData.size * canvas.grid.size);        // -Jez 23.08.02 Fix by replacing width with size 
