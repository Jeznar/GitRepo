 ...
 * Modification of the ASE module code to correct wrong size of placed storm tile.  It was correct while being p[laced but dropped 
 * to a 10 foot diameter tile (likely the default for that tile) after placement.
 * 
 * 08/01/23 0.1 Activated trace statements and isolated problem that was causing tiles to render to small and do no damage.
 *              - castTemplate.width = castTemplate.size : Sets width to tile size, handles apparent change in warpgate 
 *              - size: 2, : set the lighting strike tile to 2 tiles wide, not 3.
 *              - boltTemplate.width = boltTemplate.size : Sets width to tile size, handles apparent change in warpgate
 *              - Additional changes in utilityFunctions
 *********1*********2*********3*********4*********5*********6*********7*********8*********9*********0*********1*********2*********3*/
...
export class callLightning {
    ...
    static async createStormCloud(midiData) {
        const TL = 0;                                                           // -Jez 23.08.01
        const TAG = `callLightning_createStormCloud |`;                         // -Jez 23.08.01
        if (TL > 1) jez.trace(`${TAG} midiData: `, midiData);                   // -Jez 23.08.01
        ...
        if (TL > 1) jez.trace(`${TAG} boltDamage: `, boltDamage);                  // -Jez 23.08.01
        ...
        castTemplate.width = castTemplate.size                                  // -Jez 23.08.01 Fix: Adding this line 'fixes' the spell
        if (TL > 1) jez.trace(`${TAG} castTemplate`,castTemplate);              // -Jez 23.08.01
        let effectFile = `jb2a.call_lightning.${res}_res.${color}`;
        if (TL > 1) jez.trace(`${TAG} effectFile`,effectFile);                  // -Jez 23.08.01
        ...
        if (TL > 1) jez.trace(`${TAG} StomeTileID: `, stormTileId);                     // -Jez 23.08.01
        const updates = {
            embedded: {
                Item: {}
            }
        ...
        async function placeCloudAsTile(effectOptions) {
            const TL = 0;                                                       // -Jez 23.08.01
            const TAG = `callLightning-placeCloudAsTile |`;                     // -Jez 23.08.01
            if (TL > 0) jez.trace(`${TAG} Called`);                             // -Jez 23.08.01
    ...
    static async callLightningBolt(stormTileId, itemCardId, itemId) {
        const TL = 0;                                                           // -Jez 23.08.01
        const TAG = `callLightning-callLightningBolt |`;                        // -Jez 23.08.01
        if (TL > 1) jez.trace(`${TAG} Called`, 'stormTileId', stormTileId, 'itemCardId', itemCardId, 'itemId', itemId); // -Jez 23.08.01
        let stormCloudTile = canvas.scene.tiles.get(stormTileId);
        let getGM = game.users.find(i => i.isGM);
        let crosshairsConfig = {
            size: 2,                                                            // -Jez 23.08.01 Fix: Originally was a 3, units are squares though, so make it 2
        ...
        boltTemplate.width = boltTemplate.size                                   // -Jez 23.08.01 Fix: Adding this line 'fixes' the spell
        if (TL > 1) jez.trace(`${TAG} BoltTemplate*: `, boltTemplate);           // -Jez 23.08.01 
        ...
        if (TL > 1) jez.trace(`${TAG} TargetsInCrosshairs: `, targetsInCrosshairs);             // -Jez 23.08.01
        let casterID = stormCloudTile.getFlag("advancedspelleffects", "stormCloudTile");
        if (TL > 1) jez.trace(`${TAG} Call Lightning caster id: `, casterID);                   // -Jez 23.08.01
        let caster = canvas.tokens.get(casterID);
        let casterActor = caster.document.actor;
        const itemD = casterActor.items.get(itemId);                                
        if (TL > 1) jez.trace(`${TAG} caster: `,caster);                                        // -Jez 23.08.01
        ...
        if (TL > 1) jez.trace(`${TAG} Distance to bolt: `, dist);                               // -Jez 23.08.01
        if (dist > 60) {
        ...
        if (TL > 1) jez.trace(`${TAG} Caster Actor: `, casterActor);                            // -Jez 23.08.01
        let saveDC = casterActor.data.data.attributes.spelldc;
        if (TL > 1) jez.trace(`${TAG} Save DC: `, saveDC);
        const boltOptions = {                               
        ...
        if (TL > 1) jez.trace(`${TAG} stormCloudTile: `, stormCloudTile);                       // -Jez 23.08.01
        if (TL > 1) jez.trace(`${TAG} boltOptions:    `, boltOptions);                          // -Jez 23.08.01

        playEffect(boltTemplate, stormCloudTile, boltOptions);

        if (game.modules.get("midi-qol")?.active) {
            if (TL > 2) jez.trace(`${TAG} midi-qol is active`);              // -Jez 23.08.01
            let damage = await new Roll(`${stormCloudTile.getFlag("advancedspelleffects", "boltDamage")}`).evaluate({ async: true });
            if (TL > 2) jez.trace(`${TAG} damage: `, damage);               // -Jez 23.08.01
            if (game.modules.get("dice-so-nice")?.active) {
                game.dice3d?.showForRoll(damage);
            }

            const chatMessage = await game.messages.get(itemCardId);
            if (TL > 2) jez.trace(`${TAG} itemCardId: `, itemCardId);       // -Jez 23.08.01
            ...
            for await (let targetToken of targetsInCrosshairs) {
                if (TL > 1) jez.trace(`${TAG} damage ${targetToken.name}`, targetToken);
                ...
                if (TL > 2) jez.trace(`${TAG}saveRoll: `, saveRoll);             // -Jez 23.08.01
                newChatmessageContent.find(".midi-qol-saves-display").append($(addTokenToText(targetToken, save, saveDC, damage)));
            ...
            MidiQOL.applyTokenDamage([{ damage: damage.total, type: "lightning" }], damage.total, targetTokens, itemD, saves)

            function addTokenToText(token, roll, dc, damageRoll) {
                const TL = 0;                                                       // -Jez 23.08.01
                const TAG = `callLightning-addTokenToText |`;                       // -Jez 23.08.01
                if (TL > 1) jez.trace(`${TAG} damageRoll: `,damageRoll);            // -Jez 23.08.01
      ...
      <div>
      ${saveResult ? game.i18n.format("ASE.SavePassMessage", { saveTotal: roll, damageTotal: Math.floor(damageRoll.total / 2) }) 
                   : game.i18n.format("ASE.SaveFailMessage", { saveTotal: roll, damageTotal: damageRoll.total })}
      </div>
      <div><img src="${token?.data?.img}" height="30" style="border:0px"></div>
        ...
        async function playEffect(boltTemplate, cloud, boltOptions) {
            const TL = 0;                                               // -Jez 23.08.01
            const TAG = `callLightning-playEffect |`;                   // -Jez 23.08.01
            if (TL > 0) jez.trace(`${TAG} Called`);                     // -Jez 23.08.01
            ...
            if (TL > 1) jez.trace(`${TAG} Values `,                     // -Jez 23.08.01
                'boltStyle', boltStyle,                                 // -Jez 23.08.01
                'boltSound', boltSound,                                 // -Jez 23.08.01    
                'boltVolume', boltVolume,                               // -Jez 23.08.01
                'boltSoundDelay', boltSoundDelay,                       // -Jez 23.08.01    
                'placeCrackAsTile', placeCrackAsTile);                  // -Jez 23.08.01
            let boltEffect;
            switch (boltStyle) {
                case 'chain':
            ...
            async function placeCracksAsTile(boltTemplate, effectFilePath) {
                const TL = 0;                                                           // -Jez 23.08.01
                const TAG = `callLightning-placeCracksAsTile |`;                        // -Jez 23.08.01
                if (TL > 1) jez.trace(`${TAG} CalledTemplate Data: `, boltTemplate);    // -Jez 23.08.01
    ...
    static buildChatData(saveRolls) {
        const TL = 0;                                                           // -Jez 23.08.01
        const TAG = `callLightning-buildChatData |`;                            // -Jez 23.08.01
        if (TL > 1) jez.trace(`${TAG} saveRolls: `, saveRolls);                 // -Jez 23.08.01
        ...
        //iterate through saveRolls using for in loop
        for (let i = 0; i < saveRolls.length; i++) {
            if (TL > 2) jez.trace(`${TAG} saveRolls: `, saveRolls[i]); // -Jez 23.08.01
            ...
            if (TL > 1) jez.trace(`${TAG} Damage Roll Die Terms:     `, currDamageRollDieTerms);        // -Jez 23.08.01
            if (TL > 1) jez.trace(`${TAG} Damage Roll Numeric Terms: `, currDamageRollNumericTerms);    // -Jez 23.08.01
            ...
            if (TL > 1) jez.trace(`${TAG} currSaveRollResult: `, currSaveRollResult);     // -Jez 23.08.01
    ...
    static async handleConcentration(casterActor, casterToken, effectOptions) {
        const TL = 0;                                                               // -Jez 23.08.01
        const TAG = `callLightning-handleConcentration |`;                          // -Jez 23.08.01
        if (TL > 0) jez.trace(`${TAG} Called`);                                     // -Jez 23.08.01
        let stormCloudTiles = canvas.scene.tiles.filter((tile) => tile.data.flags.advancedspelleffects?.stormCloudTile == casterToken.id);
        if (TL > 1) jez.trace(`${TAG} casterToken: `, casterToken);                 // -Jez 23.08.01
        if (TL > 1) jez.trace(`${TAG} tiles to delete: `, [stormCloudTiles[0].id]); // -Jez 23.08.01
    ...
    static async _updateCombat(combat) {
        const TL = 0;                                                       // -Jez 23.08.01
        const TAG = `callLightning-updateCombat |`;                         // -Jez 23.08.01
        if (TL > 0) jez.trace(`${TAG} Called`);                             // -Jez 23.08.01
        ...
        if (TL > 1) jez.trace(`${TAG} update hook fired, stormCloudTiles: `, stormCloudTiles);  // -Jez 23.08.01
        if (stormCloudTiles.length > 0) {
            if (TL > 1) jez.trace(`${TAG} Detected Storm Cloud! Prompting for Bolt...`);        // -Jez 23.08.01
            let confirmData = {
    ...
    static async getRequiredSettings(currFlags) {
        const TL = 0;                                                       // -Jez 23.08.01
        const TAG = `callLightning-getRequiredSettings |`;                  // -Jez 23.08.01
        if (TL > 0) jez.trace(`${TAG} Called`);                             // -Jez 23.08.01
