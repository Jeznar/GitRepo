...
export class wallSpell extends baseSpellClass {
    ...
    async cast() {
        ...
        if (TL > 1) jez.trace(`${TAG} dimensions:`, dimensions);
        if (TL > 1) jez.trace(`${TAG} texture:   `, texture);
        if (TL > 1) jez.trace(`${TAG} type:      `, type);
        ...
        if (TL > 1) jez.trace(`${TAG} Dialog return type`, type);
        if (type == "h-panels" || type == "v-panels") {
            ...
            if (TL > 1) jez.trace(`${TAG} wallSpellPanelData:`, wallSpellPanelData);
            ...
            if (TL > 1) jez.trace(`${TAG} template data:`, this.baseTemplateData);
    ...
    static async updateCombat(combat) {
        ...
        if (TL > 1) jez.trace(`${TAG} wall templates: `, wallTemplates);
        if (wallTemplates.length && wallTemplates.length > 0) {
            for await (let wallTemplate of wallTemplates) {
                ...
                if (TL > 1) jez.trace(`${TAG} templateDocument.data: `, templateDocument.data);
                ...
                if (TL > 1) jez.trace(`${TAG} Wall template details: `, templateDetails);
                if (templateDetails.shape == CONST.MEASURED_TEMPLATE_TYPES.CIRCLE) {
                    if (TL > 1) jez.trace(`${TAG} Circle template detected...`);
                    ...
                    if (TL > 1) jez.trace(`${TAG} Side to check: `, sideToCheck);
                    ...
                    widthLoop: for (let x = startX; x < token.data.width; x++) {
                        for (let y = startY; y < token.data.height; y++) {
                            ...
                            if (inRange) {
                                ...
                                if (TL > 1) jez.trace(`${TAG} Token not in range of wall circle: `, token.name);
                            }
                    ...
                    if (TL > 1) jez.trace(`${TAG} Template Point A: `, templatePointA);
                    if (TL > 1) jez.trace(`${TAG} Template Point B: `, templatePointB);
                    const sideToCheck = templateDocument.getFlag("advancedspelleffects", "wallOperationalData.damageSide");
                    if (TL > 1) jez.trace(`${TAG} Side to check: `, sideToCheck);
                    const startX = token.data.width >= 1 ? 0.5 : (token.data.width / 2);
                    const startY = token.data.height >= 1 ? 0.5 : (token.data.height / 2);
            ...
            widthLoop: for (let x = startX; x < token.data.width; x++) {
                for (let y = startY; y < token.data.height; y++) {
                    ...
                    if (!contains) {
                        ...
                        if (TL > 1) jez.trace(`${TAG} Drag Coord Old: `, dragCoordOld);
                        if (TL > 1) jez.trace(`${TAG} Drag Coord New: `, dragCoordNew);
                        if (templateData.t == CONST.MEASURED_TEMPLATE_TYPES.CIRCLE) {
                    ...
                    if (((previousContains && contains) || (!previousContains)) && (crossed || contains)) {
                        if (TL > 1) jez.trace(`${TAG} Token touched wall!`);
                        ...
                        if (TL > 1) jez.trace(`${TAG} Token did not cross template area...`);
                    }
                }
            }
        }
    ...
    static async activateWallEffect(token, wallData) {
      ...
      <div><img src="${token?.data?.img}" height="30" style="border:0px"></div>
        ...
        if (TL > 1) jez.trace(`${TAG} Caster Token: `, casterToken);
        if (TL > 1) jez.trace(`${TAG} Caster Actor: `, casterActor);
        if (TL > 1) jez.trace(`${TAG} Spell Item:   `, spellItem);
    ...
    static async placeWalls(templateDocument, deleteOldWalls = false) {
        ...
        if (TL > 1) jez.trace(`${TAG} wallType: `, wallType);
        ...
        if (TL > 1) jez.trace(`${TAG} walls: `, walls);
    ...
    static sourceSquare(center, widthSquares, heightSquares) {
        ...
        if (TL > 1) jez.trace(`${TAG} allSpots: `, allSpots);
        return {
            x: left,
            y: top,
            center,
    ...
    static async pickFireSide(templateDocument) {
        ...
        if (TL > 1) jez.trace(`${TAG} damageSidePicked: `, damageSidePicked);
    }
    ...
    static async handleOnCast(templateDocument) {
        ...
        if (TL > 1) jez.trace(`${TAG} wallItem`, wallItem);
        if (!chatMessage) {
        ...
        if (savingThrowOnCast) {
            ...
            const targets = [];
            //------------------------------------------------------------------------------------------------------
            // Build array of tokens targeted by the cast
            //
            if (tokens.length > 0) {
                for (let i = 0; i < tokens.length; i++) {

                    const token = tokens[i];
                    if (TL > 2) jez.trace(`${TAG} Processing ${tokens[i].name} @(${token.data.x}, ${token.data.y})`);

                    const startX = token.data.width >= 1 ? 0.5 : (token.data.width / 2);
                    const startY = token.data.height >= 1 ? 0.5 : (token.data.height / 2);
                    if (TL > 2) jez.trace(`${TAG} Token sizes`, 'width',  token.data.width, 'height',  token.data.height)
                    if (TL > 2) jez.trace(`${TAG} templateDetails`, templateDetails)
                    widthLoop: for (let x = startX; x < token.data.width; x++) {
                        for (let y = startY; y < token.data.height; y++) {
                            const currGrid = {
            ...
            if (TL > 1) jez.trace(`${TAG} targets: `, targets);
            //------------------------------------------------------------------------------------------------------
            //game.user.updateTokenTargets(targets);
            if (targets.length && targets.length > 0) {
                if (TL > 1) jez.trace(`${TAG} chat message`, chatMessage);
                let chatMessageContent = await duplicate(chatMessage.data.content);
    ...
    static async placePanels(aseData, template, panelDiag, type) {
        ...
        if (TL > 1) jez.trace(`${TAG} previousTemplateData: `, previousTemplateData);
        let panelsRemaining = panelDiag.data.aseData.flags.panelCount;
        if (TL > 1) jez.trace(`${TAG} Panels Remaining:     `, panelsRemaining);
        ...
        if (TL > 1) jez.trace(`${TAG} nextTemplateData:    `, nextTemplateData);
        if (TL > 1) jez.trace(`${TAG} previousTemplateData:`, previousTemplateData);
        ...
        updateTemplateLocation = async (crosshairs) => {
            if (TL > 1) jez.trace(`${TAG} crosshairs: `, crosshairs);
            while (crosshairs.inFlight) {
                ...
                if (TL > 1) jez.trace(`${TAG} displayTemplate`, displayTemplate);
                if (!displayTemplate) return;
                const verticalTemplate = displayTemplate.data.t == CONST.MEASURED_TEMPLATE_TYPES.RAY;
                if (TL > 1) jez.trace(`${TAG} Vertical Template:`, verticalTemplate);
                let ray;
                let angle;
                if (!verticalTemplate) {
