const MACRONAME = "Dominated.0.2.js"
/*****************************************************************************************
 * 
 * 
 * Notes from Crymic:
...
async function damageCheck(workflow) {
    ...
    if (workflow.damageTotal > 0) {
        ...
        // if (TL > 2) jez.trace(`${TAG} midiWorkFlow.item.data.data.save`, midiWorkFlow.item.data.data.save)
        // if (TL > 2) console.log("")
        const SAVE_TYPE = midiWorkFlow.item.data.data.save.ability
        const SAVE_DC = midiWorkFlow.item.data.data.save.dc
        //----------------------------------------------------------------------------------------
        ...
        if (FIGHTING) {  // Oddly setting advantage to seemingly anything, including false causes it to be true
            if (TL > 2) jez.trace(`${TAG} Rolling save with advantage`, aToken.actor.data.flags["midi-qol"])
            save = (await aToken.actor.rollAbilitySave(SAVE_TYPE, { advantage: true, chatMessage: true }));
            // save = (await aToken.actor.rollAbilitySave(SAVE_TYPE, { advantage: true, flavor: FLAVOR, chatMessage: true, fastforward: true }));
        } else {
            if (TL > 2) jez.trace(`${TAG} Rolling save with out advantage`, aToken.actor.data.flags["midi-qol"])
            save = (await aToken.actor.rollAbilitySave(SAVE_TYPE, {chatMessage: true}));
            // save = (await aToken.actor.rollAbilitySave(SAVE_TYPE, { disadvantage: true }));
            // save = (await aToken.actor.rollAbilitySave(SAVE_TYPE, { disadvantage: true, flavor: FLAVOR, chatMessage: true, fastforward: true }));
        }
        ...
        // /*if (FIGHTING)*/ midiWorkFlow.advantage = FIGHTING;    if (TL>2) jez.trace(`${TAG} midiWorkFlow.advantage`, midiWorkFlow?.advantage )
        ...
        // if (TL>2) jez.trace(`${TAG} midiWorkFlow.advantageSaves 1`, midiWorkFlow.advantageSaves )
        // await midiWorkFlow.checkSaves(false); // Original line that exectuted the save
