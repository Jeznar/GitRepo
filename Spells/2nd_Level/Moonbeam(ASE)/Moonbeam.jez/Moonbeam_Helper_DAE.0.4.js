...
log("---------------------------------------------------------------------------",
    `Starting ${MACRONAME}`,"MINION_NAME");
for (let i = 0; i < args.length; i++) log(`  args[${i}]`, args[i]);

const MINION_NAME = args[1];
const VFX_NAME = args[2];
const ATTACK_ITEM = args[3];
const VFX_OUTRO = args[4];
const VFX_OPACITY = args[5] || 0.95;
const VFX_SCALE = args[6] || 0.65;

if (!MINION_NAME || !VFX_NAME || !ATTACK_ITEM || !VFX_OUTRO) {
...
log("","MINION_NAME",MINION_NAME,"VFX_NAME",VFX_NAME,"ATTACK_ITEM",ATTACK_ITEM,
    "VFX_OUTRO",VFX_OUTRO,"VFX_OPACITY",VFX_OPACITY,"VFX_SCALE",VFX_SCALE);
...
async function doEach() {
    ...
    if (!concentrating) {
        log("No longer concentrating, remove the moonbeam effect");
        let moonbeamEffect = aActor.effects.find(ef => ef.data.label === "Moonbeam") ?? null; 
        if (moonbeamEffect) {
            log("Moonbeam effect found", moonbeamEffect);
            await moonbeamEffect.delete();
        } else {
            log("Moonbeam effect was not found, this should not have happened.");
        }
    }
...
async function doOff() {
    ...
    log(`doOff ---> Delete the VFX from ${MINION_NAME}`)
    await deleteVFX(MINION_NAME);
    log(`doOff ---> Delete ${MINION_NAME}'s token`)
    await deleteToken(MINION_NAME);
 ...
 ***************************************************************************************************/
 async function deleteVFX(minion) {
    ...
    log("typeof(VFX_SCALE)", typeof(VFX_SCALE))
    new Sequence()
    .effect()
        .file(VFX_OUTRO)
        .scale(VFX_SCALE)
